...
export const uploadBackupController = async (req, res) => {
  try {
    ...
    console.log("✅ backup.json muebleria reemplazado en:", backupFilePath);
...
export const reloadBdController = async (req, res) => {
  try {
    console.log("🔄 Reiniciando base de datos (muebleria)...");
    ...
    return res.json({
      ok: true,
      message: "Base reiniciada e inicializada desde backup.json (muebleria)",
    });
  } catch (error) {
