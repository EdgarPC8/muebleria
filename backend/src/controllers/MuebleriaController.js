...
export const getCustomers = async (_req, res) => {
  const data = await Customer.findAll({ order: [["firstName", "ASC"], ["firstLastName", "ASC"], ["name", "ASC"]] });
  res.json(data);
};
...
export const updateOrder = async (req, res) => {
  const { id } = req.params;
  const { customerId, date, notes, status, receivedAt, paidAt, paymentMethod, items } = req.body;
  ...
  try {
    ...
    if (Array.isArray(items)) {
      await SaleOrderItem.destroy({ where: { orderId: order.id }, transaction: t });
      for (const row of items) {
        const product = await StoreProduct.findByPk(Number(row.productId), { transaction: t });
        if (!product) throw new Error("Producto no encontrado.");
        const qty = Number(row.quantity || 0);
        if (qty <= 0) throw new Error("Cantidad inválida.");
        const wholesaleMin = Number(product.wholesaleMinQty || 0);
    ...
    if (!wasReceived && Boolean(nextReceivedAt)) {
      ...
      const orderItems = await SaleOrderItem.findAll({ where: { orderId: order.id }, transaction: t });
      for (const item of orderItems) {
        const product = await StoreProduct.findByPk(Number(item.productId), { transaction: t });
        if (!product) throw new Error("Producto no encontrado para descontar inventario.");
        const qty = Number(item.quantity || 0);
        const currentStock = Number(product.stockBase || 0);
        if (qty <= 0) continue;
        if (currentStock < qty) {
          throw new Error(`Stock insuficiente para ${product.name}. Disponible: ${currentStock}, requerido: ${qty}.`);
        }
        await product.update({ stockBase: currentStock - qty }, { transaction: t });
        await StockMovement.create(
          {
            productId: product.id,
            type: "salida_venta",
...
export const updateSupplierOrder = async (req, res) => {
  const { id } = req.params;
  const { supplierId, date, notes, status, receivedAt, paidAt, paymentMethod, items } = req.body;
  ...
  try {
    ...
    if (!wasReceived && Boolean(nextReceivedAt)) {
      const orderItems = await SupplierOrderItem.findAll({ where: { orderId: order.id }, transaction: t });
      for (const item of orderItems) {
        const product = await StoreProduct.findByPk(Number(item.productId), { transaction: t });
        if (!product) throw new Error("Producto no encontrado para ingresar inventario.");
        const qty = Number(item.quantity || 0);
        if (qty <= 0) continue;
        const currentStock = Number(product.stockBase || 0);
        await product.update({ stockBase: currentStock + qty }, { transaction: t });
...
export const createStockAdjustment = async (req, res) => {
  const { productId, quantityBase, note, flagForReview, kind = "entrada" } = req.body;
  ...
  if (!Number.isFinite(qty) || qty <= 0) {
    return res.status(400).json({ message: "quantityBase debe ser mayor a 0." });
  }
  const t = await sequelize.transaction();
  try {
    ...
    const extra =
      typeof note === "string" && note.trim() ? ` · ${note.trim().slice(0, 400)}` : "";
...
async function openBoxForTaskPayload(payload, t, taskItemId) {
  ...
  const boxProduct = await StoreProduct.findByPk(boxProductId, { transaction: t, lock: t.LOCK.UPDATE });
  const unitProduct = await StoreProduct.findByPk(unitProductId, { transaction: t, lock: t.LOCK.UPDATE });
  if (!boxProduct || !unitProduct) throw new Error("Producto caja o unidad no encontrado.");
  if (Number(boxProduct.stockBase || 0) < boxesToOpen) throw new Error("No hay cajas suficientes para abrir.");

  const unitQty = boxesToOpen * unitsPerBox;
  await boxProduct.update({ stockBase: Number(boxProduct.stockBase || 0) - boxesToOpen }, { transaction: t });
  await unitProduct.update({ stockBase: Number(unitProduct.stockBase || 0) + unitQty }, { transaction: t });
...
export const publishTaskPlan = async (req, res) => {
  ...
  if ((plan.items || []).length === 0) return res.status(400).json({ message: "El plan no tiene tareas." });
